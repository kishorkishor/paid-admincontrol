...
function route_after_switch(array $perms, array $teams): string {
  ...
  if (isset($P['qc_supervisor_access']))    return 'qc_supervisor.php';       // QC supervisor
  if (isset($P['qc_access']))               return 'qc.php';                  // QC member
  if (isset($P['chinese_inbound_access']))  return 'chinese_inbound.php';    // Chinese Inbound
  if (isset($P['manage_ledgers']))          return 'accounts_payments_direct.php';  // Accounts
  if (isset($P['team_supervisor_access']))  return 'query_team_supervisor.php'; // Any supervisor view
  if (isset($P['view_orders']) && isset($P['view_queries'])) return 'index.php'; // mixed ops
  if (isset($P['view_queries']) || isset($P['submit_price_quote'])) return 'query_team_member.php'; // Agent
  if (isset($P['view_orders']))             return 'order_supervisor.php';    // Read-only orders
  if (isset($P['manage_admins']))           return 'users.php';               // Admin management

  // Optional: team-based overrides if you key special team IDs
  $teamIds = array_map(fn($t) => (int)($t['id'] ?? $t['team_id'] ?? 0), $teams);
  if (in_array(12, $teamIds, true)) return 'chinese_inbound.php'; // e.g., team 12 = CN Inbound
  if (in_array(13, $teamIds, true)) return 'qc.php';             // e.g., team 13 = QC
